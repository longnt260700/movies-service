...
import { IPaginateCustom } from 'src/interfaces/paginate.interface';
...
export class BaseRepository<Model extends BaseTable> extends EventEmitter {
  ...
  async update(id: number, entity: QueryDeepPartialEntity<Model>): Promise<Model> {
    return classToPlain(await this.model.update(id, entity)) as Model;
  }
  ...
  async paginationQueryBuilderCustom(
    queryBuilder: SelectQueryBuilder<Model>,
    options: IPaginateCustom<Number>,
  ) {
    const totalItems = await queryBuilder.getCount();
    const { limit, page } = options;
    const itemsInPageRaw = queryBuilder.skip(limit*(page-1)).take(limit);
    const itemsResult = await itemsInPageRaw.getMany();
    const itemCount = itemsResult.length;
